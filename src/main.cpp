...
const char* ap_ssid = "Balloon-7432";
const char* ap_password = "something123";
WebServer server(80);
...
void PIDLoop(void * pvParameters) {
    ...
    for(;;) {
        ...
        if (targetSet && dt > 0) {
            ...
            float pOut = Kp * error;
            if (abs(error) < 10.0) integral += error * dt;
...
void loop() {
    ...
    if (millis() - lastLCD > 250) {
        lcd.setCursor(0, 0);
        lcd.print("Alt: "); lcd.print(currentAltitude, 1); lcd.print("m  ");
        lcd.setCursor(0, 1);
        lcd.print(targetSet ? "Tgt: " : "Set: "); 
        lcd.print(targetSet ? targetAltitude : previewAlt, 1); lcd.print("m  ");
        lastLCD = millis();
    }
